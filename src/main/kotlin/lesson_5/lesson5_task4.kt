...
fun main() {
    ...
    if (USER_NAME == readLine()) {
        println("Введите пароль:")
        if (PASSWORD == readLine()) {
            println("Добро пожаловать, $USER_NAME!")
        } else {
            println("Пароль неверен, попробуйте еще раз.")
        }
    } else {
        println("Зарегистрируйтесь и завершите процесс.")
    }
}
...
Система должна проверить, зарегистрирован ли пользователь. Если нет, то система должна предложить
пользователю зарегистрироваться и закончить процесс тут (завершить программу).
